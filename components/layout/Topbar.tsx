...
export function Topbar() {
  ...
  const canViewNotifications = hasPermission(role ?? user?.role, Permission.NOTIFICATIONS);
  const { data: notifications } = useNotifications(canViewNotifications);
        ...
        {canViewNotifications && (
          <Button variant="outline" size="icon" className="relative bg-card" aria-label="Notifications" asChild>
            <Link href="/dashboard/notifications">
              ...
              <span className="absolute -right-1 -top-1 grid h-4 min-w-4 place-items-center rounded-full bg-primary px-1 text-[9px] font-black text-white">{notifications?.length ?? 0}</span>
            </Link>
          </Button>
        )}
        <ThemeToggle />
