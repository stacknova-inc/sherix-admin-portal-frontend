...
const statuses: Array<"All Statuses" | StaffStatus> = [
  "All Statuses",
  "Active",
  "Suspended",
  "Inactive",
];
const departments = staffRoleDefinitions.map(
  (definition) => definition.department,
);
const roleLabels = new Map(
  staffRoleDefinitions.map((definition) => [definition.role, definition.label]),
);
...
function rolesForDepartment(department: string) {
  return staffRoleDefinitions.filter(
...
function defaultRole(department: string) {
  return (
    rolesForDepartment(department)[0]?.role ?? staffRoleDefinitions[0].role
  );
}
      ...
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Content className="fixed left-1/2 top-1/2 z-50 w-[calc(100vw-2rem)] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-xl border bg-card p-5 shadow-2xl">
          <div className="flex items-start justify-between gap-4">
            <div>
              <DialogPrimitive.Title className="text-base font-black">
                {title}
...
function StaffFormModal({
  ...
  function setField<Key extends keyof CreateStaffInput>(
    key: Key,
    value: CreateStaffInput[Key],
  ) {
    setForm((current) => {
      if (key === "department") {
    ...
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="bg-white">
          ...
          <DropdownMenuItem
            onClick={() => confirmStatus("activate", "Activate")}
          >
  ...
  return [
    {
      id: "index",
      header: "#",
      cell: ({ row }) => <span className="font-black">{row.index + 1}</span>,
    },
    {
      ...
      cell: ({ row }) => (
        <PersonCell
          ...
          initials={initials(row.original.fullName)}
    ...
    { accessorKey: "phoneNumber", header: "Phone Number" },
    {
      ...
      header: "Department",
      cell: ({ row }) => (
        <SoftTag tone="blue">{row.original.department}</SoftTag>
      ),
    },
    {
      accessorKey: "role",
      ...
      cell: ({ row }) => (
        <span className="font-bold">
          {roleLabels.get(row.original.role) ?? row.original.role}
        </span>
      ),
    },
    {
    ...
    
    {
      ...
      header: "Actions",
      cell: ({ row }) => (
        <StaffActions staff={row.original} onEdit={onEdit} onToast={onToast} />
      ),
    },
  ];
}
  ...
  const filteredRows = React.useMemo(() => {
    ...
    return staff
      .filter((member) => {
        const matchesSearch =
          !search ||
          [member.fullName, member.email, member.phoneNumber].some((value) =>
    ...
    <div className="mx-auto max-w-[1600px] space-y-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <PageHeader
          ...
          subtitle="Manage Sherix internal staff accounts, roles, status changes, and audit activity."
        />
        ...
        <ToolbarCard>
          ...
          <Select value={role} onValueChange={setRole}>
            ...
            <SelectContent className="bg-white">
              {[
                "All Roles",
                ...staffRoleDefinitions.map((definition) => definition.role),
              ].map((value) => (
                <SelectItem key={value} value={value}>
          ...
          <Select
            value={sortBy}
            onValueChange={(value) => setSortBy(value as StaffSortKey)}
          >
