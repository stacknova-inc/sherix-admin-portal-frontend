...
export function ExportButton({ data, filename = "export", title }: ExportButtonProps) {
  function handleExport() {
    if (!data.length) return;

    const headers = Object.keys(data[0]);
    const reportTitle = title ?? titleCase(filename);
    ...
    const html = `<!doctype html>
      <html>
        <head>
          ...
          <style>
            @page { size: A4 landscape; margin: 16mm 12mm 18mm; }
            * { box-sizing: border-box; }
            body { margin: 0; color: #111827; font-family: Arial, Helvetica, sans-serif; font-size: 11px; }
            header { display: flex; align-items: flex-start; justify-content: space-between; gap: 24px; border-bottom: 2px solid #E30613; padding-bottom: 12px; margin-bottom: 18px; }
            .brand { font-size: 18px; font-weight: 900; letter-spacing: 0; color: #E30613; }
            h1 { margin: 6px 0 0; font-size: 22px; line-height: 1.2; }
            .meta { color: #4b5563; line-height: 1.6; text-align: right; }
            table { width: 100%; border-collapse: collapse; table-layout: fixed; }
            thead { display: table-header-group; }
            tr { page-break-inside: avoid; }
            th { background: #111827; color: #ffffff; font-size: 10px; text-align: left; text-transform: uppercase; }
            th, td { border: 1px solid #d1d5db; padding: 7px 8px; vertical-align: top; word-break: break-word; }
            tbody tr:nth-child(even) td { background: #f9fafb; }
            footer { position: fixed; bottom: -10mm; left: 0; right: 0; display: flex; justify-content: space-between; color: #6b7280; font-size: 9px; border-top: 1px solid #e5e7eb; padding-top: 6px; }
        ...
        <body>
          <header>
            <div>
              <div class="brand">SHERIX ADMIN PORTAL</div>
              <h1>${escapeHtml(reportTitle)}</h1>
            </div>
            <div class="meta">
              <div>Export date: ${escapeHtml(exportedAt.toLocaleString())}</div>
              <div>Total rows: ${data.length}</div>
            </div>
          </header>
          <table>
            <thead>
              <tr>${headers.map((header) => `<th>${escapeHtml(titleCase(header))}</th>`).join("")}</tr>
            </thead>
            <tbody>
              ${data
          ...
          <footer>
            <span>Generated by Sherix Admin Portal</span>
            <span class="page"></span>
          </footer>
          <script>
            window.addEventListener("load", () => {
              document.title = ${JSON.stringify(`${filename}-${exportedAt.toISOString().slice(0, 10)}.pdf`)};
              window.focus();
              window.print();
            });
          </script>
